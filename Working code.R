...
blankmap <- ggplot(data = world) +
  geom_sf(fill = "white", color = "white") +  #land and land borders
  coord_sf(xlim = c(E_limit, W_limit), ylim = c(S_limit, N_limit), expand = FALSE) +
  labs(x = "Longitude", y = "Latitude") +
  theme(
    panel.background = element_rect(fill = "white", color = NA),  #ocean
    panel.grid = element_blank(),
    panel.border = element_rect(color = "white", fill = NA),  # frame around map
    axis.text = element_text(color = "white"), #text
    axis.title = element_text(color = "white") #text
  )
...
markedmap <- blankmap + 
  geom_point(data = ___FILTERED_DATA_NAME___, aes(x = long, y = lat), color = "white", size = 2) +
  geom_text(
    data = ___FILTERED_DATA_NAME___,
...
new_E_limit <- -  # Eastern limit, KEEP THE NEGATIVE SIGN "-"
new_W_limit <- -   # Western limit, KEEP THE NEGATIVE SIGN "-"
new_S_limit <-    # Southern limit
new_N_limit <-    # Northern limit
...
finalmap <- ggplot(data = world) +
  geom_sf(fill = "white", color = "white") + # Land and land borders colours
  coord_sf(xlim = c(new_E_limit, new_W_limit), ylim = c(new_S_limit, new_N_limit), expand = FALSE) +
  labs(x = "Longitude", y = "Latitude") +
  theme(
    panel.background = element_rect(fill = "white", color = NA), #water colour
    panel.grid = element_blank(),
    panel.border = element_rect(color = "white", fill = NA), #map border
    axis.text = element_text(color = "white"), #text
    axis.title = element_text(color = "white") #text
  ) +
  
  geom_point(data = ___FILTERED_DATA_NAME___, aes(x = long, y = lat), color = "white", size = 3) + #points on the map
  geom_text(
    data = ___FILTERED_DATA_NAME___,
    aes(x = long, y = lat, label = date),
    nudge_y = 0.5,
    size = 3,
    color = "white", # date labels
    fontface = "bold"
  )
